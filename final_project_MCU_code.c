 ...
 *
 * ACTUATORS:
 *   Buzzer      – Smoke / temperature alerts only (GPIO, PTC8)
 *   Water Pump  – Temperature control, relay (GPIO, PTD2)
 *   Fan         – Ventilation, relay (GPIO, PTD3)
 *
 * DISPLAY:
 ...
 *
 * ACTUATOR BEHAVIOUR (rev 3):
 *   Fan:   ON at 26°C. OFF below 25°C (hysteresis). Immediately OFF when
 *          temperature reaches 27°C (pump takes over). Fan and pump never
 *          run simultaneously.
 *   Pump:  ON at 27°C. Fan is shut down first (100 ms relay settle delay).
 *          OFF when temperature drops below 26°C.
 *   Flame: DISPLAY ONLY. The KY-026 flame sensor updates the OLED
 *          ("FLAME:DET") but does NOT trigger any actuator or buzzer.
 *          This prevents false pump/buzzer activations from the sensor's
 *          floating DO pin.
 *   Buzzer: Responds to smoke (MQ-2) and high temperature only.
 *
 * I2C0 PIN-MUX SHARING:
 *   MLX90614 uses PTB2/PTB3; FXOS8700CQ uses PTD8/PTD9. The idle pair is
 *   tristated to prevent bus contention. A mutex (g_i2c0_owner) avoids
 *   redundant pin switching on consecutive calls to the same device.
 *
 * MQ-2 PRESENCE DETECTION:
 *   ADC-based (AO pin). The DO pin is open-collector active-LOW; the K66F
 *   internal pull-down fought the module's pull-up causing mq2_ok to always
 *   read false. Fix: sensor present when AO > 50 mV (unconnected = ~0 mV).
 *
 * FLAME FALSE-TRIGGER FIX:
 *   3-second confirmation counter. KY-026 DO floats HIGH with a weak
 *   pull-down, making flame_present=true. ADC then reads ~0 mV on a
 *   floating channel (< FLAME_DETECT_MV) — a false confirm. Requiring 3
 *   consecutive agreeing samples eliminates single-cycle artefacts.
 *
 * AI DECLARATION:
 *   All inline comments and the file header were written with assistance from
 *   Claude (Anthropic) for documentation and grammar purposes. All hardware
 *   decisions, pin assignments, firmware logic, and bug fixes were designed
 *   and implemented by the project team. AI assisted with debugging.
 *   [Claude – AI used for commenting, documentation, and debugging assistance]
 * =============================================================================
 */
...
/*
 * Fan / Pump temperature thresholds:
 *
 *  <25°C  – both OFF
 *  25–25°C – transitional (fan hysteresis off-point)
 *  26°C   – Fan ON, Pump OFF
 *  27°C+  – Fan OFF, Pump ON
 *
 * FAN_ON_TEMP_C  (26): fan activates at this temperature
 * FAN_OFF_TEMP_C (25): fan deactivates below this temperature (hysteresis)
 * PUMP_ON_TEMP_C (27): pump activates at this temperature (fan turns off first)
 * PUMP_OFF_TEMP_C(26): pump deactivates below this temperature
 */
#define FAN_ON_TEMP_C       26U
#define FAN_OFF_TEMP_C      25U
#define PUMP_ON_TEMP_C      27U
#define PUMP_OFF_TEMP_C     26U
...
/* MQ-2 ADC presence floor — unconnected channel reads ~0 mV */
#define MQ2_PRESENT_MV      50U
...
/* =============================================================================
   Section 4: DWT Cycle-Counter Timing
   MUST be called before any delay_ms() — see main() boot sequence note.
   ============================================================================= */
...
static void LED_Blink(uint32_t n, uint32_t ms) {
    for (uint32_t i = 0; i < n; i++) { LED_On(); delay_ms(ms); LED_Off(); delay_ms(ms); }
}

/* =============================================================================
   Section 6: Shared Pin Configurations
   ============================================================================= */
...
/* Pull-up for open-collector active-LOW DO lines (MQ-2) */
static const port_pin_config_t k_pullup_pin = {
    kPORT_PullUp, kPORT_SlowSlewRate,
    kPORT_PassiveFilterDisable, kPORT_OpenDrainDisable,
    kPORT_LowDriveStrength, kPORT_MuxAsGpio, kPORT_UnlockRegister
};

/* I2C: open-drain + pull-up per I2C specification */
...
/* Tristate: high-impedance, disconnects idle I2C0 pair from bus */
...
/* =============================================================================
   Section 7: I2C0 Pin-Mux Switching (Mutex-Protected)
   ============================================================================= */
...
/* =============================================================================
   Section 8: Actuator & Sensor GPIO
   ============================================================================= */
...
static void InitGPIO(void) {
    ...
    /* MQ-2 DO: pull-up (open-collector active-LOW); presence detected via ADC */
    PORT_SetPinConfig(MQ2_DO_PORT,   MQ2_DO_PIN,   &k_pullup_pin);
    GPIO_PinInit(MQ2_DO_GPIO,        MQ2_DO_PIN,   &in);

    /* MQ-135 and Flame DO: pull-down (active-HIGH modules) */
...
/* =============================================================================
   ...
   BUZ_SOLID > BUZ_BEEP > BUZ_OFF.
   Triggered only by smoke and temperature — flame does NOT trigger the buzzer.
   ============================================================================= */
...
/* =============================================================================
   Section 10: ADC1 — Init and Read
   12-bit, software trigger, 32-sample hardware average.
   ADC_ReadMV and MQ2_Present are placed here (not forward-declared) so
   MQ2_Present can call ADC_ReadMV without a prototype.
   ============================================================================= */
...
/*
 * MQ-2 presence: ADC-based. Defined after ADC_ReadMV — no forward declaration
 * needed. An unconnected ADC channel reads ~0 mV; a live MQ-2 always outputs
 * above MQ2_PRESENT_MV (50 mV) even in completely clean air.
 */
static bool MQ2_Present(void) {
    return ADC_ReadMV(MQ2_AO_CH) > MQ2_PRESENT_MV;
}
...
/* =============================================================================
   Section 11: DHT11 Bit-Bang Driver
   Dual-read (60 ms apart); second result preferred. Eliminates checksum
   failures caused by bus capacitance slowing edges on long wire runs.
   ============================================================================= */
...
/* =============================================================================
   Section 12: I2C Helper Functions
   ============================================================================= */
...
/* =============================================================================
   Section 13: MLX90614 IR Thermometer
   I2C0 via PTB2/PTB3. Result in tenths of °C (e.g. 236 = 23.6°C).
   ============================================================================= */
...
/* =============================================================================
   Section 14: FXOS8700CQ Accelerometer
   I2C0 via PTD8/PTD9. ±2g range, 14-bit, 4096 LSB/g.
   ============================================================================= */
...
/* =============================================================================
   ...
   I2C1 (PTC10/PTC11). Custom 5×7 font.
   Page layout:
     Page 0: T:xxC  H:xx%
     Page 2: IR:xx.xC  AQ:GOOD
     Page 4: S:NRM  FL:DET   ← flame display-only, no actuator
...
/*
 * OLED_Update — rate-limited to 100 ms intervals.
 *
 * Page 0: T:xxC  H:xx%
 * Page 2: IR:xx.xC  AQ:GOOD
 * Page 4: S:NRM  FL:DET    ← "FL:DET" = flame confirmed (display only)
 * Page 6: RADAR:YES / RADAR:NO
 */
static void OLED_Update(uint8_t temp, uint8_t hum, bool dht_ok,
    ...
    /* Page 2: IR temperature and Air Quality */
    oled_clear_page(2);
    ...
    /*
     * Page 4: Smoke level and Flame status.
     * Flame is DISPLAY ONLY — "FL:DET" is purely informational.
     * No actuator or buzzer is triggered by flame detection.
     */
    oled_clear_page(4);
    const char *fl_txt = !flame_present ? "NO SEN"
                       : flame          ? "DET"
                                        : "NO";
    if (mq2_ok) snprintf(ln, sizeof(ln), "S:%s FL:%s      ", smoke, fl_txt);
    else        snprintf(ln, sizeof(ln), "S:ND  FL:%s     ", fl_txt);
    oled_print(4, 0, ln);

    /* Page 6: Radar presence */
    oled_clear_page(6);
...
/* =============================================================================
   Section 16: UART1 — ESP8266 JSON Telemetry
   ============================================================================= */
...
/* =============================================================================
   ...
   Boot sequence:
     1. DWT_Init  — MUST be first (all delay_ms depend on it)
     2. Board clocks and GPIO
     3. OLED init (halts with blink pattern on failure)
     4. I2C0 tristated → MLX90614 → FXOS8700CQ
     5. UART1 → ESP8266
     6. Main loop

   Actuator logic:
     <26°C  → Fan OFF, Pump OFF
      26°C  → Fan ON,  Pump OFF
     ≥27°C  → Fan OFF (100 ms relay settle), Pump ON
     <26°C  → Pump OFF (when falling from pump zone)
     <25°C  → Fan OFF  (when falling from fan zone, hysteresis)
     Flame  → OLED only, zero actuator response
     Buzzer → smoke danger = SOLID; smoke warn / temp warn = BEEP; else OFF
   ============================================================================= */

int main(void) {
    /* Step 1: DWT MUST be first — delay_ms relies on it */
    DWT_Init(); delay_ms(INIT_DELAY_MS);
    BOARD_InitBootPins(); BOARD_InitBootClocks();
    DWT_Init(); /* Re-initialise after clock change to reset cycle counter */
    ...
    bool     fan_on        = false;
    bool     flame         = false;
    bool     flame_present = false;
    bool     mq2_ok        = false;
    bool     mq135_ok      = false;
    uint8_t  flame_confirm = 0;
    ...
    for (;;) {
        ...
        if (now - last_sample_ms >= 1000U) {
            ...
            /* ── Flame Detection — DISPLAY ONLY ─────────────────────────────
             * Requires 3 consecutive seconds of ADC confirmation before
             * flame=true. A single glitch resets the counter.
             * flame=true affects ONLY the OLED and JSON telemetry.
             * It does NOT activate the pump, fan, or buzzer.
             * ─────────────────────────────────────────────────────────────── */
            if (flame_present) {
                if (ADC_ReadMV(FLAME_AO_CH) < FLAME_DETECT_MV) {
                    if (flame_confirm < 3U) flame_confirm++;
                } else {
                    flame_confirm = 0;
                }
            } else {
            ...
            if (ok) {
                /* Exponential moving average on humidity (alpha = 0.25) */
                ...
                /* ── Fan & Pump — Temperature Only, Mutually Exclusive ──────
                 *
                 * Three distinct temperature zones:
                 *
                 *  Zone A  (temp < FAN_ON_TEMP_C = 26°C)
                 *    → Both OFF. Apply hysteresis when falling:
                 *      fan turns off below FAN_OFF_TEMP_C  (25°C)
                 *      pump turns off below PUMP_OFF_TEMP_C (26°C)
                 *
                 *  Zone B  (FAN_ON_TEMP_C ≤ temp < PUMP_ON_TEMP_C = 26°C exactly)
                 *    → Fan ON, Pump OFF.
                 *
                 *  Zone C  (temp ≥ PUMP_ON_TEMP_C = 27°C)
                 *    → Fan OFF (100 ms relay settle), Pump ON.
                 *
                 * Flame detection has NO effect on either actuator.
                 * ─────────────────────────────────────────────────────────── */

                if (last_temp >= PUMP_ON_TEMP_C) {
                    /* Zone C: pump takes over — ensure fan is off first */
                    if (fan_on) {
                        fan_on = false; Fan_Set(false);
                        delay_ms(100); /* Relay settle before pump closes */
                    }
                    if (!pump_on) { pump_on = true; Pump_Set(true); }

                } else if (last_temp >= FAN_ON_TEMP_C) {
                    /* Zone B: fan only — ensure pump is off */
                    if (pump_on) { pump_on = false; Pump_Set(false); }
                    if (!fan_on) { fan_on  = true;  Fan_Set(true);  }

                } else {
                    /* Zone A: below fan threshold — shut down with hysteresis */
                    if (pump_on && last_temp < PUMP_OFF_TEMP_C) {
                        pump_on = false; Pump_Set(false);
                    }
                    if (fan_on  && last_temp < FAN_OFF_TEMP_C) {
                        fan_on  = false; Fan_Set(false);
                    }
                }

            } else {
                /* DHT11 failed — safe state */
            ...
            OLED_Update(last_temp, hum_show, last_dht_ok,
                        ...
                        flame_present, flame, g_mlx_ok, ir_temp, g_radar);
            ...
            /* ── Buzzer — Smoke & Temperature Only ──────────────────────────
             * Flame does NOT trigger the buzzer. Removing flame from buzzer
             * logic eliminates false alarms from the floating DO pin.
             * ─────────────────────────────────────────────────────────────── */
            bool danger = (mq2_ok && smoke_lvl == S_DNG)
